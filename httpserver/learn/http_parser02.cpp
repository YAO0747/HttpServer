...
//当浏览器出现连接重置时，可能是网站根目录出错或者http响应格式
//出错或访问的文件内容完全为空
...
//服务器分析请求行
HTTP_CODE parse_requestline(char*text,CHECK_STATE& checkstate)
{
	//'\t'(制表符)或者空格用于分隔http的内容
	//检查请求行中text是否有\t或者空格，若无,则http请求语法出错
			  ...
			  checkstate,int &read_index,int &start_line)
{
	...
	{
		char*text = buffer + start_line;//start_line是行在buffer中的位置
		...
		switch(checkstate)
		{
		   case CHECK_STATE_REQUESTLINE://第一个状态,分析请求行
			{
				retcode = parse_requestline(text,checkstate);
